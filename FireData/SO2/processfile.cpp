...
void openfile(const char *file ) {
  ...
  string ofilename = "output_"+namecity.substr(0,namecity.size()-1) +"_"+namecountry+"_annual.txt";;
  ...
  out_file <<"year \t number of month \tSO2_PBL \t SO2_TRL \t SO2_TRM\n";
  /*
  ...
  int stryear = vso2[0]->iyear;
  int strmon = vso2[0]->imon;
  int nummon = 1 ; 
  //out_file <<stryear<<" \t";
  float tot1 = 0.0f, tot2 = 0.0f, tot3 = 0.0f;
  int cnt1 = 0, cnt2 =0, cnt3 = 0;
  for(int i = 0 ; i < vso2.size(); i ++) {
    //out_file <<vso2[i]->iyear<<"_"<<vso2[i]->imon<<" \t";
    if(stryear  == vso2[i]->iyear ){
      if(strmon != vso2[i]->imon) {
	nummon ++;
	strmon = vso2[i]->imon;
      }
      for(int j =0 ; j < vso2[i]->SO2_PBL.size() ; j++) {
	tot1 += vso2[i]->SO2_PBL[j];
	cnt1 ++;
      }
      for(int j =0 ; j < vso2[i]->SO2_TRL.size() ; j++) {
	tot2 += vso2[i]->SO2_TRL[j];
	cnt2 ++;
      }
      for(int j =0 ; j < vso2[i]->SO2_TRM.size() ; j++) {
	tot3 += vso2[i]->SO2_TRM[j];
	cnt3 ++;
      }
    } else {
      out_file << stryear <<" \t"<<nummon <<" \t"<< tot1/cnt1 <<" \t"<<tot2/cnt2<<" \t"<<tot3/cnt3<<" \t\n";
      nummon = 1;
      stryear  = vso2[i]->iyear;
      strmon = vso2[i]->imon;
      tot1 = 0;
      tot2 = 0;
      tot3 = 0;
